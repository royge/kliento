...
pub fn extract_valid_bill_info_from(sheet: Range<DataType>) -> Vec<BillInfo> {
    let mut row_data: Vec<BillInfo> = Vec::new();

    let today: DateTime<Local> = Local::now();

    for row in sheet.rows() {
        let mut data: BillInfo = BillInfo {
...
pub fn upload_bill_info(bill_info: Vec<BillInfo>, config: &Config) -> Result<(), Box<dyn Error>> {
    let client = reqwest::blocking::Client::new();

    let len = bill_info.len();
    ...
    for (index, bill) in bill_info.iter().enumerate() {
        data.push(bill);
